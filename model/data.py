...
class ImageResizer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        updated = np.array(
            list(map(lambda x: resize(x, (self.height,
                                          self.width,
                                          self.channels),
                                      mode='constant',
                                      preserve_range=True), X))
        )
        return updated
...
class DataReader(object):
    ...
    def _dataset(self):
        all_images = glob(os.path.join(
            self.datapath, '{0}_*'.format(self.stage), '*', '*', '*'))
    ...
    def read_and_stack(self, images):
        return 1. / 255.0 * np.sum(
            np.stack([imread(c_img) for c_img in images], 0), 0)
...
class TrainDataReader(DataReader):
    ...
    def read(self, column="train"):
        ...
        output = dataset[dataset.TrainingSplit == column]
        output = pd.merge(
            output[output.ImageType == "images"].rename(
                index=str, columns={"path": "images"}),
            pd.DataFrame(
                output[output.ImageType == "masks"].groupby("ImageId")[
                    "path"].apply(list)
...
class ResultSaver(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, out_columns=["ImageId", "EncodedPixels"],
                 ofile="output.csv"):
